...
Deno.test("fortune", async () => {
  ...
  const result = await execute({}, [command], "$fortune()")
...
Deno.test("good girl", async () => {
  ...
  const result = await execute(makeFields("$good girl"), [util, good], "$good('girl')")
  assertEquals(omit(result, ["output"]), { success: true, value: undefined })
...
Deno.test("randint", async () => {
  ...
  const result = await execute(makeFields(".randint 1 10"), [util, command], "$randint()")
  assertEquals(omit(result, ["output"]), { success: true, value: undefined })
...
Deno.test("pingall", async () => {
  ...
  const result = await execute({}, [command], "$pingall()")
...
Deno.test("sha256", async () => {
  ...
  const result = await execute({ args }, [sha256Command], `$sha256(...args)`)
